...
ConfigureRsyslog() {
    if [ ! -f ${RSYSLOG_DEST} ]; then
        ...
        cat ${RSYSLOG_TEMP} | sed "s/%WORKSPACE_ID%/${WORKSPACE_ID}/g" | sed "s/%PORT%/${PORT}/g" > ${RSYSLOG_DEST}
        ...
        egrep -q "@127.0.0.1:${PORT}" ${RSYSLOG_DEST}
        if [ $? -ne 0 ]; then
            echo "Configuring rsyslog for OMS logging"
...
UnconfigureRsyslog() {
    if [ -f ${RSYSLOG_DEST} ]; then
        ...
        egrep -q "OMS Syslog collection for ${WORKSPACE_ID}|@127.0.0.1:${PORT}" ${RSYSLOG_DEST}
        if [ $? -eq 0 ]; then
            cp ${RSYSLOG_DEST} ${RSYSLOG_DEST}.bak
            egrep -v "OMS Syslog collection for ${WORKSPACE_ID}|@127.0.0.1:${PORT}" ${RSYSLOG_DEST}.bak > ${RSYSLOG_DEST}
...
ConfigureOldRsyslog() {
    ...
    egrep -q "@127.0.0.1:${PORT}" ${OLD_RSYSLOG_DEST}
    if [ $? -ne 0 ]; then
        echo "Configuring (old) rsyslog for OMS logging"

        cat ${RSYSLOG_TEMP} | sed "s/%WORKSPACE_ID%/${WORKSPACE_ID}/g" | sed "s/%PORT%/${PORT}/g" >> ${OLD_RSYSLOG_DEST}
        RestartService rsyslog
    fi
}

UnconfigureOldRsyslog() {
    egrep -q "OMS Syslog collection for ${WORKSPACE_ID}|@127.0.0.1:${PORT}" ${OLD_RSYSLOG_DEST}
    if [ $? -eq 0 ]; then
        echo "Unconfiguring (old) rsyslog for OMS logging"

        cp ${OLD_RSYSLOG_DEST} ${OLD_RSYSLOG_DEST}.bak
        egrep -v "OMS Syslog collection for ${WORKSPACE_ID}|@127.0.0.1:${PORT}" ${OLD_RSYSLOG_DEST}.bak > ${OLD_RSYSLOG_DEST}
...
ConfigureSyslog_ng() {
    ...
    if [ $? -ne 0 ]; then
        ...
        cat ${SYSLOG_NG_TEMP} | sed "s/%SOURCE%/${source}/g" | sed "s/%WORKSPACE_ID%/${WORKSPACE_ID}/g" | sed "s/%PORT%/${PORT}/g" >> ${SYSLOG_NG_DEST}
...
SetVariables() {
    ...
    if [ -z $WORKSPACE_ID -o -z $PORT ]; then
        echo "WORKSPACE_ID and PORT are required" 1>&2
        exit 1
    fi
}
